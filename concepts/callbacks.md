# NEAR Protocol Cross-Contract Callbacks Security Guide

## Overview: Cross-Contract Calls in NEAR
...
**Key Concepts:** cross-contract calls, callbacks, asynchronous execution, reentrancy attacks, security vulnerabilities, NEAR Protocol, smart contract security
...
**Manual Implementation**: Check `env::predecessor_account_id() == env::current_account_id()`

**Related Terms**: predecessor, current_account, access control, method visibility, callback security
...
**Related Terms**: attached deposit, NEAR tokens, refund, gas allocation, transaction failure, user funds protection
...
**Related Terms**: reentrancy, race condition, state consistency, deposit pattern, staking, validator, balance management, security best practices
...
- How do I secure callback methods in NEAR?
- What happens to user funds if a cross-contract call fails?
- How do I prevent reentrancy attacks in NEAR contracts?
- Why should I delay balance updates until callbacks?
- How do I handle refunds in cross-contract calls?
- What is the predecessor in NEAR callbacks?